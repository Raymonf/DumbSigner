...
struct EntitlementValue {
    ...
    EntitlementValue(Type type, std::string value)
        : type(type), length(value.length()), value(value) { }
...
struct EntitlementBlob {
    ...
    EntitlementBlob(std::string key, std::string v)
        : key(EntitlementValue::Type::String, key)
    {
    ...
    EntitlementBlob(std::string key, bool v)
        : key(EntitlementValue::Type::String, key)
    {
    ...
    EntitlementBlob(std::string key, std::vector<std::string> values)
        : key(EntitlementValue::Type::String, key), isArray(true)
    {
...
struct EntitlementsSuperBlob {
    ...
    EntitlementsSuperBlob(std::vector<EntitlementBlob> blobs)
        : blobs(blobs)
    {
...
struct AlgorithmSHA1 : Algorithm {
    AlgorithmSHA1()
...
struct AlgorithmSHA256 : Algorithm {
    AlgorithmSHA256()
...
class HashProxy : public HashBuffer {
private:
...
Bundle Sign(const std::string& root, Folder& folder, const std::string& key, std::map<std::string, Hash>& remote, const std::string& requirement, const Functor<std::string(const std::string&, const std::string&)>& alter, const Functor<void(const std::string&)>& progress, const Functor<void(double)>& percent)
{
    ...
    folder.Find("", fun([&](const std::string& name) {
        ...
        bundles[nested[1]] = Sign(bundle, subfolder, key, local, "", Starts(name, "PlugIns\\") ? alter : static_cast<const Functor<std::string(const std::string&, const std::string&)>&>(fun([&](const std::string&, const std::string& entitlements) -> std::string { return entitlements; }))
            , progress, percent);
